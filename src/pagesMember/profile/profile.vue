...
import { postUpdateProfileAPI } from '@/services/profile'
...
// 获取个人信息
// 数据没有返回之前，profile 为空，双向绑定访问空对象的属性就会报错
// 所以赋值一个空对象
...
const onGenderChange = (e: UniHelper.RadioGroupOnChangeEvent) => {
    profile.value.gender = e.detail.value as '男' | '女'
}
let regionCode: string[] = []
const onRegionChange = (e: UniHelper.RegionPickerOnCancelEvent) => {
    profile.value!.fullLocation = e.target!.value.join(' ')
    regionCode = e.target!.code
}
...
const onUpdateProfile = async () => {
    ...
    const res = await postUpdateProfileAPI({
...
<template>
    <view class="viewport">
        ...
        <view class="form">
            ...
            <!-- 提交按钮 -->
            <button class="form-button" @tap="onUpdateProfile">保 存</button>
        </view>
    </view>
</template>
