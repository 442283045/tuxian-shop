...
import { useLoadData } from '@/composables'
import {
...
// 猜你喜欢
const { guessRef, loadMoreData } = useLoadData()
// 弹出层组件
...
const getOrderDetailData = async (id: string) => {
    ...
    if (
        ...
        ].includes(res.result.orderState)
    ) {
...
const onOrderPay = async () => {
    ...
    // } else {
    //     // 正常环境微信支付
    //     const res = await getPayWxPayMinPayAPI({ orderId: query.id })
...
const onOrderConfirm = () => {
    // 二次确认弹窗
    uni.showModal({
        content: '为保证您的权益，请收到或并确认无误后再确认收货',
    ...
    >
        <template v-if="order">
            ...
            >
                <!-- 待付款状态:展示支付按钮 -->
                <template v-if="order.orderState === OrderState.DaiFuKuan">
                    <view @tap="onOrderPay" class="button primary">
                        去支付
                    </view>
